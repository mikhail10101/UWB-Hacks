    ...
    <>
      <div className="h-screen w-screen flex flex-col items-center justify-center bg-[#EEE2DC]">
        ...
        <div className="minw-1/2 min-h-1/4 flex items-center justify-around flex-wrap">
          {arr.map((i) => (
            <Link href={`/play/${category}/${i}`} key={i} className="ease-out duration-500 hover:-translate-y-2">
              <div className="text-2xl w-[15rem] h-[3.25rem] bg-[#AC3B61] outline outline-1 rounded-md flex items-center justify-center text-1xl text-white font-serif gap-[1rem]">
                <p className="m-0">{i}</p>
              </div>
            </Link>
          ))}
